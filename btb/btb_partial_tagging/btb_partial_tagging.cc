...
#define BASIC_INDIRECT_BTB_SETS 1024 // Nishant
#define BASIC_BTB_WAYS 8
#define BASIC_INDIRECT_BTB_SIZE 8096 // Nishant
...
struct BASIC_INDIRECT_BTB_ENTRY { // Nishant
  uint16_t ip_tag;
  uint64_t target;
  uint64_t lru;
};
...
BASIC_INDIRECT_BTB_ENTRY basic_indirect_btb[NUM_CPUS][BASIC_INDIRECT_BTB_SETS][BASIC_BTB_WAYS]; // Nishant
uint64_t basic_btb_conditional_history[NUM_CPUS];
uint64_t basic_indirect_btb_lru_counter[NUM_CPUS]; // Nishant
...
uint16_t basic_btb_indirect_hash(uint8_t cpu, uint64_t ip) // Nishant
{
  uint16_t hash = (ip) ^ (basic_btb_conditional_history[cpu]);
  return (hash);
}

uint64_t basic_indirect_btb_set_index(uint64_t ip) { return ((ip >> 2) & (BASIC_INDIRECT_BTB_SETS - 1)); } // Nishant

BASIC_INDIRECT_BTB_ENTRY* basic_indirect_btb_find_entry(uint8_t cpu, uint64_t ip) // Nishant
{
  uint64_t set = basic_indirect_btb_set_index(ip);
  for (uint32_t i = 0; i < BASIC_BTB_WAYS; i++) {
    if (basic_indirect_btb[cpu][set][i].ip_tag == basic_btb_indirect_hash(cpu, ip)) {
      return &(basic_indirect_btb[cpu][set][i]);
    }
  }
...
BASIC_INDIRECT_BTB_ENTRY* basic_indirect_btb_get_lru_entry(uint8_t cpu, uint64_t set) // Nishant
{
  uint32_t lru_way = 0;
  uint64_t lru_value = basic_indirect_btb[cpu][set][lru_way].lru;
  for (uint32_t i = 0; i < BASIC_BTB_WAYS; i++) {
    if (basic_indirect_btb[cpu][set][i].lru < lru_value) {
      lru_way = i;
      lru_value = basic_indirect_btb[cpu][set][lru_way].lru;
    }
  }

  return &(basic_indirect_btb[cpu][set][lru_way]);
}
...
void basic_indirect_btb_update_lru(uint8_t cpu, BASIC_INDIRECT_BTB_ENTRY* indirect_btb_entry) // Nishant
{
  indirect_btb_entry->lru = basic_indirect_btb_lru_counter[cpu];
  basic_indirect_btb_lru_counter[cpu]++;
}
...
{
  std::cout << "Basic BTB sets: " << BASIC_BTB_SETS << " ways: " << BASIC_BTB_WAYS << " indirect buffer size: " << BASIC_INDIRECT_BTB_SIZE
  ...
  for (uint32_t i = 0; i < BASIC_INDIRECT_BTB_SETS; i++) {
    for (uint32_t j = 0; j < BASIC_BTB_WAYS; j++) {
      basic_indirect_btb[cpu][i][j].ip_tag = 0;
      basic_indirect_btb[cpu][i][j].target = 0;
      basic_indirect_btb[cpu][i][j].lru = 0;
    }
  }
  basic_indirect_btb_lru_counter[cpu] = 0;
...
std::pair<uint64_t, uint8_t> O3_CPU::btb_prediction(uint64_t ip, uint8_t branch_type)
{
  ...
  if (branch_type == BRANCH_RETURN) {
    ...
    auto indirect_btb_entry = basic_indirect_btb_find_entry(cpu, ip);

    if (indirect_btb_entry == NULL) {
    ...
    basic_indirect_btb_update_lru(cpu, indirect_btb_entry);

    return std::make_pair(indirect_btb_entry->target, always_taken);
   ...
   // use BTB
    ...
    if (indirect_btb_entry == NULL) {
      if ((branch_target != 0) && taken) {
        ...
        uint64_t set = basic_indirect_btb_set_index(ip);
        auto repl_entry = basic_indirect_btb_get_lru_entry(cpu, set);

        repl_entry->ip_tag = basic_btb_indirect_hash(cpu, ip);
        repl_entry->target = branch_target;
        //repl_entry->always_taken = 1;
        basic_indirect_btb_update_lru(cpu, repl_entry);
      }
    } else {
      // update an existing entry
      indirect_btb_entry->target = branch_target;
    }
  }
  if (branch_type == BRANCH_CONDITIONAL) {
