...
#define BASIC_INDIRECT_BTB_SETS 256 // Nishant
#define BASIC_BTB_WAYS 8
#define BASIC_INDIRECT_BTB_SIZE 2048 // Nishant
...
uint64_t basic_btb_indirect_hash(uint8_t cpu, uint64_t ip) // Nishant
{
  uint64_t hash = (ip) ^ (basic_btb_conditional_history[cpu]);
  return (hash);
}
